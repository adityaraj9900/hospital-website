    ...
    <section id="contact" className="py-24 lg:py-32 bg-[#0a0f1e] relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        <div className="grid lg:grid-cols-[1fr_480px] gap-16 xl:gap-24 items-start">
          ...
          >
            <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-blue-400 mb-5 block">
              Book Appointment
            </span>
            <h2 className="text-4xl lg:text-5xl font-extrabold text-white tracking-tight leading-[1.1] mb-6">
              Start your journey<br />to better health<br />
              <span className="grad-blue">today.</span>
            </h2>
            <p className="text-zinc-400 text-[15.5px] leading-relaxed mb-10 max-w-sm">
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={t} className="flex items-start gap-4">
                  <div className="w-11 h-11 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center shrink-0 mt-0.5">
                    <Icon className="w-5 h-5 text-blue-400" />
                  </div>
                  <div>
                    <p className="text-white font-semibold text-[14.5px]">{t}</p>
                    <p className="text-zinc-500 text-xs mt-0.5">{s}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {sent ? (
              ...
              >
                ...
                <p className="text-zinc-500 mb-8 text-[15px]">
                  We&apos;ll confirm your appointment within 2 hours by email and phone.
                </p>
                <button
                  onClick={() => setSent(false)}
                  className="bg-[#0066cc] text-white font-semibold px-8 py-3 rounded-xl hover:bg-blue-700 transition-colors"
                >
                  Book Another
                </button>
              </motion.div>
              ...
              <form
                ...
                className="bg-white rounded-3xl p-8 lg:p-9 space-y-4 shadow-2xl shadow-black/30"
              >
                <div className="mb-2">
                  <h3 className="text-xl font-bold text-zinc-900">Patient Details</h3>
                  <p className="text-zinc-400 text-sm mt-0.5">All fields are required.</p>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {([
                    ...
                    <label key={n} className="relative block">
                      ...
                      <input
                        type={t} name={n} placeholder={ph} required
                        value={f[n as keyof typeof f]} onChange={ch}
                        className="w-full pl-10 pr-4 py-3 text-sm rounded-xl border border-zinc-200 text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-[#0066cc]/30 focus:border-[#0066cc] transition-colors"
                      />
                    </label>
                  ))}
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <label className="relative block">
                    ...
                    <select
                      name="dept" required value={f.dept} onChange={ch}
                      className="w-full pl-10 pr-4 py-3 text-sm rounded-xl border border-zinc-200 text-zinc-700 focus:outline-none focus:ring-2 focus:ring-[#0066cc]/30 focus:border-[#0066cc] appearance-none bg-white transition-colors"
                    >
                  ...
                  <label className="relative block">
                    ...
                    <input
                      type="date" name="date" required value={f.date} onChange={ch}
                      className="w-full pl-10 pr-4 py-3 text-sm rounded-xl border border-zinc-200 text-zinc-700 focus:outline-none focus:ring-2 focus:ring-[#0066cc]/30 focus:border-[#0066cc] transition-colors"
                    />
                  </label>
                </div>
                ...
                <p className="text-center text-xs text-zinc-400">
                  HIPAA compliant · End-to-end encrypted · Never shared
                </p>
              </form>
            )}
          </motion.div>
