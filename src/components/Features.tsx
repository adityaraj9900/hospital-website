...
const features = [
  {
    ...
    stat: { val: "12,000+", label: "Robotic procedures" },
    dark: false,
  },
  {
    ...
    stat: { val: "99.4%", label: "Diagnostic accuracy" },
    dark: true,
  },
  {
  ...
  return (
    <section id="services" className="py-6">
      {features.map((f, i) => (
        <div
          key={f.tag}
          className={`${f.dark ? "bg-[#0a0f1e]" : "bg-white"}`}
        >
          <div className="max-w-7xl mx-auto px-6 lg:px-10 py-20 lg:py-28">
            <div className={`grid lg:grid-cols-2 gap-12 xl:gap-20 items-center ${i % 2 === 1 ? "lg:grid-flow-dense" : ""}`}>
              ...
              >
                <span className={`inline-block text-xs font-bold uppercase tracking-[0.15em] mb-5 ${f.dark ? "text-blue-400" : "text-[#0066cc]"}`}>
                  {f.tag}
                </span>
                <h2 className={`text-4xl lg:text-5xl font-extrabold leading-[1.08] tracking-tight mb-6 ${f.dark ? "text-white" : "text-zinc-900"}`}>
                  {f.title}
                </h2>
                <p className={`text-[16px] leading-relaxed mb-8 ${f.dark ? "text-zinc-400" : "text-zinc-500"}`}>
                  {f.body}
                </p>
                ...
                <div className={`inline-flex items-center gap-4 rounded-2xl px-6 py-4 mb-8 ${f.dark ? "bg-white/5 border border-white/10" : "bg-blue-50 border border-blue-100"}`}>
                  <div>
                    <p className={`text-3xl font-extrabold leading-none ${f.dark ? "text-white" : "text-zinc-900"}`}>{f.stat.val}</p>
                    <p className={`text-xs mt-1 font-medium ${f.dark ? "text-zinc-400" : "text-zinc-500"}`}>{f.stat.label}</p>
                  </div>
                </div>
                ...
                <motion.a
                  href="#contact"
                  whileHover={{ x: 4 }}
                  className={`inline-flex items-center gap-2 font-semibold text-[15px] ${f.dark ? "text-blue-400 hover:text-blue-300" : "text-[#0066cc] hover:text-blue-700"} transition-colors`}
                >
              ...
              <motion.div
                ...
                className={`relative h-[400px] lg:h-[480px] rounded-3xl overflow-hidden shadow-2xl ${i % 2 === 1 ? "lg:col-start-1" : ""}`}
              >
                <Image src={f.image} alt={f.title} fill className="object-cover hover:scale-105 transition-transform duration-700" sizes="(max-width:1024px) 100vw, 50vw" />
                <div className={`absolute inset-0 ${f.dark ? "bg-gradient-to-tr from-[#0a0f1e]/40 to-transparent" : "bg-gradient-to-tr from-zinc-900/20 to-transparent"}`} />
              </motion.div>
